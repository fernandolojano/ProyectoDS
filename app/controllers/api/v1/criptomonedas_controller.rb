...
class CriptomonedasController < ApplicationController
  ...
  def index
    ...
    query = "SELECT criptomonedas.*, vH.valor valor_anterior FROM criptomonedas, valor_historicos vH where criptomonedas.token_criptomoneda = vH.token_criptomoneda AND vH.fecha in (select max(fecha) from valor_historicos vH2 where vH2.token_criptomoneda = criptomonedas.token_criptomoneda)"
    @criptomonedas = ActiveRecord::Base.connection.execute(query)
